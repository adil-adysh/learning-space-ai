...
function ModalContainer($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (store_get($$store_subs ??= {}, "$stack", stack).length > 0) {
      $$renderer2.push("<!--[-->");
      ...
      const each_array = ensure_array_like(store_get($$store_subs ??= {}, "$stack", stack));
      for (let idx = 0, $$length = each_array.length; idx < $$length; idx++) {
        let item = each_array[idx];
        $$renderer2.push(`<div${attr_class(`modal-pane ${stringify(idx === store_get($$store_subs ??= {}, "$stack", stack).length - 1 ? "top" : "hidden")}`, "svelte-6t6cxg")}${attr("aria-hidden", idx === store_get($$store_subs ??= {}, "$stack", stack).length - 1 ? "false" : "true")}>`);
        if (item.component) {
          $$renderer2.push("<!--[-->");
          $$renderer2.push(`<!---->`);
